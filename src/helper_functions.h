...
/*
 * Computes square of the Euclidean distance between two 2D points.
...
inline double gausian2D(double dx, double sigma_x, double dy, double sigma_y){
    return 1.0/(2*M_PI*sigma_x*sigma_y) * exp(-dx*dx / (2*sigma_x*sigma_x))* exp(-dy*dy / (2*sigma_y*sigma_y));
}
